    ...
    const struct DDS_TypeAllocationParams_t * alloc_params) 
{
    ...
    sample = new (std::nothrow) Accident ;
    if (sample == NULL) {
        return NULL;
    }

    if (!Accident_initialize_w_params(sample,alloc_params)) {
        delete  sample;
        sample=NULL;
    }
    return sample;
...
AccidentPluginSupport_destroy_data_w_params(
    ...
    Accident_finalize_w_params(sample,dealloc_params);
...
AccidentPluginSupport_copy_data(
    ...
    const Accident *src)
{
    return Accident_copy(dst,(const Accident*) src);
}
...
AccidentPlugin_return_sample(
    ...
    void *handle)
{
    Accident_finalize_optional_members(sample, RTI_TRUE);
...
RTIBool
AccidentPlugin_serialize_to_cdr_buffer_ex(
    char *buffer,
    unsigned int *length,
    const Accident *sample,
    DDS_DataRepresentationId_t representation)
{
    ...
    Accident_get_typecode();
    pd.programs = AccidentPlugin_get_programs();
    if (pd.programs == NULL) {
        return RTI_FALSE;
    }
...
AccidentPlugin_serialize_to_cdr_buffer(
    ...
    const Accident *sample)
{
    return AccidentPlugin_serialize_to_cdr_buffer_ex(
        buffer,
        length,
        sample,
    ...
    const struct DDS_PrintFormatProperty *property)
{
    ...
    if (!AccidentPlugin_serialize_to_cdr_buffer(
        NULL, 
        &length, 
        sample)) {
...
unsigned int
AccidentPlugin_get_serialized_key_max_size_for_keyhash(
    PRESTypePluginEndpointData endpoint_data,
    RTIEncapsulationId encapsulation_id,
    unsigned int current_alignment)
{
...
struct RTIXCdrInterpreterPrograms *AccidentPlugin_get_programs()
{
...
{ 
    ...
    Accident_finalize_optional_members;
